...
import { getSupplyChains, deleteSupplyChainById } from 'apiSdk/supply-chains';
import { SupplyChainInterface } from 'interfaces/supply-chain';

type ColumnType = ColumnDef<SupplyChainInterface, unknown>;
...
export function SupplyChainListPage(props: SupplyChainListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SupplyChainInterface>>(
    () => `/supply-chains?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteSupplyChainById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: SupplyChainInterface) => {
    if (hasAccess('supply_chain', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/supply-chains/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/supply-chains/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('supply_chain', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/supply-chains/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('supply_chain', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      ...
      >
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="supply_chain" />
        </Flex>

        {hasAccess('supply_chain', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/supply-chains/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
